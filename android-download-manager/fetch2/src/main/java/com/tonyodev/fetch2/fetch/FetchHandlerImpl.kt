...
class FetchHandlerImpl(private val namespace: String,
    ...
    override fun retry(ids: IntArray): List<Download> {
        ...
        val downloadInfoList = databaseManager.get(ids.toList()).filterNotNull()
        downloadInfoList.forEach {
            //(thuan): We allow retry any request
    ...
    override fun getDownloadsLimit(limit: Int, columnCreated: Long, queryText: String, selectedFileType: FileTypeValue): List<Download> {
        throwExceptionIfClosed()
        return databaseManager.getLimit(limit, columnCreated, queryText, selectedFileType)
    }
