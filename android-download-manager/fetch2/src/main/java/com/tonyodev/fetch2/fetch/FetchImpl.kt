...
open class FetchImpl constructor(override val namespace: String,
    ...
    override fun getDownloadsLimit(limit: Int, createdDate: Long, queryText: String, selectedFileType: FileTypeValue, func: Func<List<Download>>) {
        synchronized(lock) {
            fetchHandler.throwExceptionIfClosed()
            handler.post {
                try {
                    val downloads = fetchHandler.getDownloadsLimit(limit, createdDate, queryText, selectedFileType)
                    uiHandler.post {
                        func.call(downloads)
                    }
                } catch (e: FetchException) {
